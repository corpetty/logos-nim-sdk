# logos_sdk — a Nim SDK for Logos modules

The Nim counterpart of [`logos-rust-sdk`](https://github.com/logos-co/logos-rust-sdk):
the runtime behind typed, generated clients a **Nim** Logos module uses to call
other modules, plus the module-side context/emit/token seam its generated surface
delegates to, plus a LIDL→Nim code generator that emits both.

It exists to (1) stop every Nim module from hand-rolling its own `lp_*` binding +
bytes codec + proxy (muster did — `module/src/transport/lp_ffi.nim`,
`delivery.nim`), and (2) give the Logos platform a Nim SDK on par with the Rust and
C++ ones ahead of the LIDL→CDDL / new-core transition, so a Nim module's callable
surface stays generated as that contract layer moves.

## Layout (mirrors logos-rust-sdk)

```
  src/logos_sdk.nim            umbrella export
  src/logos_sdk/
    ffi.nim      lp_* C-ABI bindings            (rust src/ffi.rs)
    bytes.nim    {"_bytes":<b64url>} codec       (rust src/bytes.rs)   — pure, tested
    error.nim    LogosCallError (raising client) (rust src/error.rs)
    plugin.nim   PluginProxy: shared-per-target client, sync/async call, subscribe
                                                 (rust src/plugin.rs)
    api.nim      protocol version + save_token; module context + emit + cstring seam
                                                 (rust src/api.rs)
  lidl-gen/
    gen.nim      pure JsonNode→Nim: genProvider + genClient    — headless, tested
    lidl_gen.nim CLI: lidl_c.h parse bridge over gen.nim       (rust lidl-gen/)
  tests/
    tbytes.nim   codec vectors + round-trips
    tgen.nim     generator output + naming
```

## The three generated outputs

A `.lidl` contract yields, exactly as the Rust generator does for the first two:

**Provider surface** (`lidl_gen provider contract.lidl out.nim`) — the module's own
side: the seven `logos_module_*` C exports + the dispatch table + the method
descriptor, delegating context/emit/token/protocol to this runtime. The author
writes one `proc <stem><Method>*(...)` per contract method (`stem` is the module
name without `_module`, e.g. `muster_module` → `musterHealth` — the same convention
muster hand-wrote, now generated, and the prefix namespaces the author's impl away
from system identifiers so a method named `echo` or `add` doesn't collide).

**Consumer client** (`lidl_gen client contract.lidl out.nim [target]`) — a typed
`PluginProxy` wrapper per dependency: each contract method becomes **two** Nim
procs that encode the args (a `bstr` via `bytesArg`, scalars as JSON), call, and
decode the result into the Nim return type:
- `<method>` — lenient: a typed zero-value on any failure, never raises.
- `<method>OrRaise` — raising: a `LogosCallError` on a transport/method error or a
  result that doesn't decode to the contract's declared type.

This is the piece muster's own `tools/lidl_gen.nim` never had — muster called
`delivery_module` through a hand-written proxy (until it migrated onto this SDK).

**Driver manifest** (`lidl_gen driver contract.lidl out.nim [coordinatable-csv] [tier1-csv]`)
— the "module → Muster driver" derivation (muster epic exo-fa4, P-D5). Every Logos
module method that lets a user *do something* can become a coordinated Muster intent;
this emits, per curated action:
- the **effect schema** (`{module, method, argSchema}`) and its dCBOR **domain tag**
  `muster.invoke.<module>.<method>.v1` — byte-exact with muster's invoke driver, so a
  manifest-derived effect and muster's runtime effect canonicalize identically (inv 5),
- a **capability name** (`<module>.<method>`) — the Basecamp app-to-app routing key
  (`namespace.verb`), kept **separate** from the domain tag: the capability name is
  stable while the schema id migrates to a cdCDDLe root when that ratifies, so an action
  is addressable both as a Basecamp capability and as a CDDL-typed effect,
- the **intent-propose card** copy — the LIDL `description` (the `@brief`) becomes the
  action label, each param names a field (LIDL params carry no description, so the
  field label is the param name — honest, not a fabricated sentence),
- for **Tier 0** (the generic `invoke` driver): the config a room registers, no code,
- for **Tier 1** (a module-native auth/signing model, like Safe): a `Driver` **skeleton**
  to complete, with `canonicalize`/`verifyContribution` flagged "implement against the
  module's signed form" and the Safe driver named as the worked example.

Only the *mechanical* parts are derived. The three things LIDL can't tell you — **which**
methods are coordinatable actions, the **authorization** model, and what **"done"** means
— are declared, never guessed: `coordinatable-csv` curates the actions in (omit it and
the read-pruning heuristic marks candidates *uncurated*, to be confirmed), `tier1-csv`
names the module-native ones. The manifest is a compile-time JSON-string const plus a
parsed node; Tier-1 skeletons ride along as Nim block comments so the file still compiles.

## Using it

```nim
import logos_sdk

# hand-written, or use a generated client
let counter = newPluginProxy("counter_module", origin = "my_module")
let r = counter.callSync("increment", args(%1))
if r.ok: echo r.value          # the result JSON

# bytes cross the wire in the shared tagged form
let blob = counter.callSync("store", args(bytesArg(@[byte 1, 2, 3])))
```

## Type mapping (LIDL → Nim)

| LIDL   | consumer arg / return | provider author seam | wire |
|--------|-----------------------|----------------------|------|
| `tstr` | `string`              | `string`             | JSON string |
| `bstr` | `seq[byte]`           | `string` (getStr)    | `{"_bytes":<b64url>}` |
| `int`  | `int`                 | `int`                | JSON int |
| `uint` | `int`                 | `int`                | JSON int |
| `bool` | `bool`                | `bool`               | JSON bool |
| other  | `JsonNode`            | `string`             | JSON |

## Testing

```bash
cd sdk && nimble test          # tbytes + tgen, headless
```

The runtime's FFI (`ffi`/`plugin`/`api`) binds `lp_*` symbols that resolve only at
plugin **link** time, so those modules don't `nim r` standalone; they `nim check`
clean, and the generator's pure core + the codec are fully unit-tested. The repo's
generator tests also `nim check` the *generated* provider and client against this
runtime, so the emitted code is proven to compile.

## Status

- **Done:** runtime (ffi/bytes/error/plugin/api), generator (provider + consumer
  client with lenient + raising twins, **+ driver manifest**), 28 headless tests,
  generated-code compile check.
- **First consumer:** the [muster](https://github.com/corpetty/muster) project —
  its module surface is generated by `lidl-gen` and its lp_* runtime is
  `logos_sdk/{ffi,bytes}`. A module consumes this SDK by adding it as a Nim package
  dependency (its build's `--path`) and importing `logos_sdk/*`.

## Consuming from a logos-module-builder module

The builder's Nim import path comes from `codegen.nim.packages` in the module's
`metadata.json` (github `{owner, repo, rev, hash}` entries added with `--path`).
Add this repo there with `"subdir": "src"`, so `import logos_sdk/ffi` resolves:

```json
{"owner": "corpetty", "repo": "logos-nim-sdk", "rev": "<commit>", "hash": "<sri>", "subdir": "src"}
```

Regenerate a module's surface with `lidl-gen` in `provider` mode (and a typed
client per dependency in `client` mode) — see the recipe in the module's own
`tools/regen.sh`.
